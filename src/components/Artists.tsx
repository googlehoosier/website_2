...
const VenueSpaces = () => {
  ...
  const handleBooking = (venueId: number) => {
    window.location.href = `/payment?venue=${venueId}`;
  };
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {venue.slots.map((slot, index) => {
                      const isBooked = Math.random() < 0.3; // Simulating booked slots
                      return (
                        <div 
                          key={index} 
                          className={`text-gray-300 text-sm flex items-center gap-2 bg-gray-700/30 rounded-lg p-2 ${
                            isBooked ? 'line-through opacity-50' : ''
                          }`}
                        >
                          <span className={`w-2 h-2 rounded-full ${isBooked ? 'bg-gray-500' : 'bg-pink-500'}`}></span>
                          {slot}
                        </div>
                      );
                    })}
